    ...
    <>
      ...
      {!isOpen && !isCartOpen && (
        <button
          ...
          className="print:hidden fixed bottom-24 right-4 md:bottom-6 md:right-6 z-[60] h-14 w-14 rounded-full shadow-2xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 bg-[#FF8800] text-white hover:bg-[#FF7700] animate-bounce-subtle"
        >
          <Sparkles className="h-7 w-7" />
        </button>
      )}

      {/* Chat Window - also hidden when cart is open and when printing */}
      {isOpen && !isCartOpen && (
        <div className="print:hidden fixed bottom-20 right-4 md:bottom-6 md:right-6 z-[70] w-[calc(100vw-2rem)] md:w-[calc(100vw-3rem)] max-w-[450px] h-[75vh] max-h-[calc(100vh-120px)] md:max-h-[650px] shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-300 rounded-3xl border border-white/20 dark:border-gray-800 bg-white/90 dark:bg-gray-950/90 backdrop-blur-xl">
