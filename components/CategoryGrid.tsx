...
export function CategoryGrid() {
  ...
  return (
    <section className="py-1 pb-2 md:pb-4 max-w-7xl mx-auto">
      ...
      {/* Mobile/Tablet: horizontal scroll that bleeds to screen edges */}
      <div className="lg:hidden -mx-4 px-4 flex flex-row gap-4 pb-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth">
        {CATEGORIES.map((cat) => (
          <button
            key={cat.id}
      ...
      {/* Desktop: 10 columns grid layout */}
      <div className="hidden lg:grid lg:grid-cols-10 gap-4 xl:gap-6 px-4">
        {CATEGORIES.map((cat) => (
          <button
            key={cat.id}
            onClick={() => handleCategoryClick(cat.id)}
